...
eqtl_pipeline $ARGS $targets -n -q
if [ $? -eq 0 ]; then
    echo skipping submission of jobs...
else
    submit_jobs eqtl1_$JOBNAME_SUF  "$PREV" eqtl_pipeline $ARGS
...
if [ $? -eq 0 ]; then
    ...
    submit_jobs eqtl2_$JOBNAME_SUF "$PREV" eqtl_pipeline $ARGS
...
if [ $? -eq 0 ]; then
    ...
    submit_jobs eqtl3_$JOBNAME_SUF "$PREV" eqtl_pipeline $ARGS
...
if [ $? -eq 0 ]; then
    ...
    submit_jobs eqtl4_$JOBNAME_SUF "$PREV" eqtl_pipeline $ARGS
...
if [ $? -eq 0 ]; then
    ...
    submit_jobs eqtl5_$JOBNAME_SUF "$PREV" eqtl_pipeline $ARGS
...
if [ $? -eq 0 ]; then
    ...
    submit_jobs eqtl6_$JOBNAME_SUF "$PREV" eqtl_pipeline $ARGS
...
if [ $? -eq 0 ]; then
    ...
    submit_jobs eqtl7_$JOBNAME_SUF "$PREV" eqtl_pipeline $ARGS
    PREV="eqtl7_$JOBNAME_SUF*"
fi


echo "set of jobs 8..."
targets=`eqtl_pipeline $* targets8 | tail -n 1`
eqtl_pipeline $ARGS $targets -n -q
if [ $? -eq 0 ]; then
    echo skipping  wave of jobs...
else
    submit_jobs eqtl8_$JOBNAME_SUF "$PREV" eqtl_pipeline $ARGS
    PREV="eqtl8_$JOBNAME_SUF*"
fi


echo "set of jobs 9..."
targets=`eqtl_pipeline $* targets9 | tail -n 1`
submit_jobs eqtl9_$JOBNAME_SUF "$PREV" eqtl_pipeline $ARGS

# final job
targets=step4
submit_job_get_email  eqtl8_$JOBNAME_SUF "eqtl9_$JOBNAME_SUF*" eqtl_pipeline $ARGS
